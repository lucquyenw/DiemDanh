...
Java_com_example_pc_attendance_Recognition_SupportVectorMachine_jniSvmTrain(JNIEnv *env, jobject obj, jstring cmdIn) {
	const char *cmd = env->GetStringUTFChars(cmdIn, 0);
	debug("jniSvmTrain cmd = %s", cmd);
...
JNIEXPORT void JNICALL
Java_com_example_pc_attendance_Recognition_SupportVectorMachine_jniSvmPredict(JNIEnv *env, jobject obj, jstring cmdIn) {
	const char *cmd = env->GetStringUTFChars(cmdIn, 0);
	debug("jniSvmPredict cmd = %s", cmd);
